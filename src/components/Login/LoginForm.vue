<template>
  <section id="login-form" class="login-page__login-form login-form --center-to-left">
    ...
    <p class="login-form__subtitle login-form__subtitle--muted">
      Войдите в свою учетную запись, чтобы продолжить создавать и редактировать свои задачи.
    </p>
    <form
      class="login-form__form"
      action=""
      @submit.prevent="$emit('submitForm', mailUser, passwordUser)"
    >
      <LoginInput
        v-model="mailUser"
        :idInput="'login-mail'"
      ...
      <button :disabled="!isCorrect" class="login-form__btn" type="submit" id="login-btn">
        Войти
      </button>
    </form>
  </section>
