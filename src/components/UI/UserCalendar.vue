<template>
  <section class="calendar">
    <i @click="shiftMonth(-1)" class="calendar__button fa-solid fa-angles-left"></i>
    <table class="calendar__table">
      <thead class="calendar__header">
        <tr>
          <th v-for="(day, index) in weekDays" :key="index" class="calendar__table-header">
            {{ day }}
          </th>
        </tr>
      </thead>
      <tbody class="calendar__body">
        <tr v-for="(week, weekIndex) in calendarDays" :key="weekIndex" class="calendar__table-row">
          <td
            ...
            class="table-data"
          >
            <p class="calendar__day" @click="setDate(day.date)">
              {{ day.date.getDate() }}
            </p>
          </td>
        </tr>
    ...
    <i @click="shiftMonth(1)" class="calendar__button fa-solid fa-angles-right"></i>
  </section>
</template>
...
const calendarDays = computed(() => {
  ...
  const end = endOfWeek(addDays(endOfWeek(currentDate.value, { weekStartsOn: 1 }), 1), {
    weekStartsOn: 1
  })
