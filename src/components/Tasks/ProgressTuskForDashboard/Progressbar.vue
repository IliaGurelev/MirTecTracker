  ...
  <aside class="task-details">
    ...
    <div v-for="(barData, index) in progressBarsData" :key="index" class="tag-progress">
      <div class="tag-progress">
        <p>
          {{ barData.label }} <span>{{ barData.completedCount }} / {{ barData.openCount }}</span>
        </p>
        <div
          class="progress-bar__progress"
          :style="{ width: 100 + '%', backgroundColor: barData.color }"
        >
          <div
            class="progress"
            :style="{ width: barData.progress + '%', backgroundColor: barData.bcolor }"
          ></div>
        </div>
      </div>
    </div>
...
const progressBarsData = computed(() => {
  ...
  uniqueBriefcases.forEach((name) => {
    ...
    const completedTasks = props.items.filter(
      (item) => item.briefcase.name === name && item.status === 'close'
    ).length
