<template>
  <div class="task-list">
    <div v-if="props.tasks.length === 0" class="task-list__message">
      <p class="task-list__text">На этот день ничего не запланировано</p>
    </div>
    <ul class="task-list__list">
      <li v-for="(task, index) in filteredTasks" :key="task.id" class="task-list__item">
        <div class="task-list__mark-wrap">
          <div :class="{ 'task-list__mark--active': index === 0 }" class="task-list__mark"></div>
        </div>
        <DiaryTask
          :isActive="index === 0"
          :task="task"
