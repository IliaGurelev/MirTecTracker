  ...
  >
    <div class="project-column-heading">
      <h2 class="project-column-heading__title">
        {{ column.title }}<span class="kanban__move-icon">{{ getCount(column) }}</span>
      </h2>
    </div>
    <Task
...
const getList = (status) =>
  props.items ? props.items.filter((item) => item.status === status) : []
...
const deleteTask = () => {
  taskStore.deleteTask(taskIdToDelete.value)
  hideModal()
}
