<template>
  <section class="popup">
    <form class="popup__form" action="" @submit.prevent="$emit('submitForm', dataTask)">
      <input v-model="nameTask" class="popup__input" type="text" placeholder="Название" required />
      <textarea v-model="descriptionTask" class="popup__textarea" placeholder="Описание"></textarea>
      <div class="popup__wrapper">
        <input v-model="timeTask" class="popup__input popup__input--pointer" type="time" required />
        <input v-model="dateTask" class="popup__input popup__input--pointer" type="date" required />
        <input class="popup__button" value="Добавить" type="submit" />
      </div>
    </form>
  </section>
</template>
