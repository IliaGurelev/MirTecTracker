      ...
      <li v-if="props.tasksList.length === 0">
        <div class="task-list__message">
          <p class="task-list__text">
            Задач нету, скорее добавьте
            <RouterLink class="task-list__link" :to="{ name: 'Dashboard' }">здесь</RouterLink>
          </p>
        </div>
      </li>
      <li v-for="task in props.tasksList" :key="task.id" class="task-list__item">
        <router-link
          :to="{ name: 'Dashboard', query: { id: task.dashboardId } }"
          @click="toggleSidebar"
        >
          <DetailTask :task="task" class="task-list__task" @click="$emit('clickTask', task)" />
        </router-link>
      </li>
    </ul>
  </div>
