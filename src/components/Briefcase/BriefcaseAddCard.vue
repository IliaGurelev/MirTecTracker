<template>
  <div v-if="!isAcive" @click="switchIsActive" class="add-card">
    <i class="add-card__icon fa-solid fa-plus"></i>
  </div>
  <form
    v-else-if="isAcive"
    action=""
    @submit.prevent="$emit('submitForm', briefcaseData), switchIsActive()"
  >
    <section class="briefcase" :class="'briefcase--' + currentColor">
      <div class="briefcase__wrap--relative">
        <input
          ...
          maxlength="14"
        />
        <CharacterLimitWarning :limit="14" :length="nameBriefcase.length" />
      </div>
      <div class="briefcase__wrapper">
        <ColorPicker class="briefcase__color" @clickColor="switchColor" />
        <div class="briefcase__wrapper">
          <button @click="switchIsActive" class="briefcase__button">
            <i class="fa-solid fa-xmark"></i>
          </button>
          <button type="submit" class="briefcase__button">
